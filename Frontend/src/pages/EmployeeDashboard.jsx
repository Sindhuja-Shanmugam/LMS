...
import TeamEmp from "../pages/TeamEmp";
...
const handleLeaveSubmit = async (e) => {
  ...
  const leave_type_id = leaveTypeMap[leaveForm.type];
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert("❌ Failed to submit leave: " + errorData.message);
    }
  } catch (err) {
...
useEffect(() => {
  ...
  fetch(`http://localhost:5000/api/v1/employees/${employee.manager_id}/name`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
    ...
    .then((data) => {
      ...
      // If the response is just a string (not an object), handle accordingly:
      const name = typeof data === 'string' ? data : data.name;
      setManagerName(name || "Not Assigned");
    })
    .catch((err) => {
      console.error("Error fetching manager name:", err);
      setManagerName("Error");
    });
}, [employee]);
